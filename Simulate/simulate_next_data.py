"""
MCAR simulation runner using next simulated dataset for generating imputed datasets.

Features
- Loads the next simulated dataset from `Simulate/Data/Inputs/next_initial_simulated_data.csv`.
- Applies MCAR masking at specified rates to predictor variables (x1, x2, x3) only.
- Keeps outcome variable (y) complete throughout the simulation.
...
Notes
- All predictor variables (x1, x2, x3) are continuous and treated as such.
- Outcome variable (y) is never masked - remains complete for all observations.
...
# Input data (next simulated dataset)
INPUT_CSV = os.path.join(SCRIPT_DIR, "Data", "Inputs", "next_initial_simulated_data.csv")

# Output root
OUTPUT_ROOT = os.path.join(SCRIPT_DIR, "Data", "Imputations_NextData")

# Scenarios (MCAR only)
MCAR_RATES = [0.1, 0.2, 0.3, 0.5]  # vary this list if needed, e.g., [0.1, 0.2, 0.3]

# Core MICE parameters
M_LIST = [5, 10, 20]         # number of imputations per configuration
...
CART_MIN_SAMPLES_LEAF_LIST = [1, 5, 10]  # Good values for 1000-row dataset
# Additional CART parameters to try
CART_MAX_DEPTH_LIST: List[Optional[int]] = [None, 5, 10]
CART_CCP_ALPHA_LIST = [0, 1e-3]

RF_N_ESTIMATORS_LIST = [50, 100]  # Good values for 1000-row dataset
# Additional RF parameters to try
RF_MIN_SAMPLES_LEAF_LIST = [1, 5, 10]  # Good values for 1000-row dataset
RF_MAX_DEPTH_LIST: List[Optional[int]] = [None, 5, 10]

# Replicates per scenario/config (distinct MCAR masks)
REPLICATES = 500  # Good number for robust statistics
...
def make_mcar_mask(df: pd.DataFrame, rate: float, columns: List[str], seed: int) -> pd.DataFrame:
    ...
    Only applies masking to specified columns, leaves other columns untouched.
    """
...
def run_multi_imputation(
    ...
    # Map same method to all columns (but only columns with missing values will be imputed)
...
def main() -> None:
    ...
    print(f"Loaded next simulated dataset with shape: {full.shape}")
    print(f"Columns: {list(full.columns)}")
    print(f"Data types:\n{full.dtypes}")
    print(f"Missing values per column:\n{full.isnull().sum()}")

    # Define predictor and outcome columns
    predictor_columns = ["x1", "x2", "x3"]  # Only these will have missing values introduced
    outcome_column = "y"  # This will remain complete
    
    # Verify columns exist
    missing_cols = [col for col in predictor_columns + [outcome_column] if col not in full.columns]
    if missing_cols:
        raise ValueError(f"Missing expected columns: {missing_cols}")

    # Columns to mask under MCAR (only predictors, not outcome)
    target_columns = predictor_columns

    print(f"Will introduce missingness only in: {target_columns}")
    print(f"Will keep complete: {outcome_column}")
    ...
    print(f"Starting next simulated data simulation with {total_configs} configurations across {len(MCAR_RATES)} MCAR scenario(s)")
                            ...
                            for max_depth in CART_MAX_DEPTH_LIST:
                                for ccp_alpha in CART_CCP_ALPHA_LIST:
                                    ...
                                    for rep in rep_progress:
                                        ...
                                        # Verify outcome column is still complete
                                        assert not masked[outcome_column].isnull().any(), f"Outcome column {outcome_column} should remain complete"

                                        # Run multi-imputation in a single call
                                        imp_paths: List[str] = []
                                        meta = {
                                            "input_csv": INPUT_CSV,
                                            "dataset": "NextSimulatedData",
                                            ...
                                            "original_shape": list(full.shape),
                                            "original_columns": list(full.columns),
                                            "predictor_columns": predictor_columns,
                                            "outcome_column": outcome_column,
                                            "target_columns_for_missingness": target_columns,
                                        }
                                        ...
                                        for i, completed_df in enumerate(datasets, start=1):
                                            # Verify outcome column remains complete in imputed data
                                            assert not completed_df[outcome_column].isnull().any(), f"Outcome column {outcome_column} should remain complete in imputation {i}"
    ...
    print("Next simulated data simulation completed!")
